...
class MosaicMVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, ndepths=300, interval_scale=1.06, **kwargs):
        super(MosaicMVSDataset, self).__init__()
    ...
    def build_list(self):
        ...
        interval_scale_dict = {}
        self.scale_factors_min = {}
        self.scale_factors_max = {}
        # scans
        for scan in scans:
            ...
            pair_file = "{}/pair_mosaic.txt".format(scan)
            # read the pair file
    ...
    def read_cam_file(self, scan, filename, interval_scale):
        ...
        ##########################setting1##############################################3
        depth_target_min=float(3) #scene2,3=5
        # depth_target_min=float(lines[11].split()[0])
        # depth_target_max = float(lines[11].split()[3])
        depth_target_max=float(40) #scene1,2,3=40
       ######################################################################################
        if len(lines[11].split()) >= 3:
            depth_min=depth_target_min
            depth_max=depth_target_max
    ...
    def read_depth_scaled(self, scan, filename):
    ...
    def scale_mvs_input(self, img, intrinsics, max_w, max_h, base=32):
        ...
        if h > max_h or w > max_w: #args: --max_h=864, args_w=1152
            scale = 1.0 * max_h / h
            if scale * w > max_w:
                scale = 1.0 * max_w / w
        ...
        return img, intrinsics, int(new_w), int(new_h)
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            img_filename = os.path.join(self.datapath, '{}/images_post/Image{:0>3}.jpg'.format(scan, vid+1))

            if not os.path.exists(img_filename):
                img_filename = os.path.join(self.datapath, '{}/images/Image{:0>3}.jpg'.format(scan, vid+1))

            proj_mat_filename = os.path.join(self.datapath, '{}/cams/{:0>8}_cam.txt'.format(scan, vid))

            img = self.read_img(img_filename)
            intrinsics, extrinsics, depth_min, depth_interval= self.read_cam_file(scan, proj_mat_filename,
                                                                interval_scale=self.interval_scale[scene_name])

            # scale input
            img, intrinsics, new_w, new_h = self.scale_mvs_input(img, intrinsics, self.max_w, self.max_h)
