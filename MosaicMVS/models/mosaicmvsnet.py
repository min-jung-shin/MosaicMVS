...
class DepthNet(nn.Module):
    ...
    def forward(self, features, proj_matrices, depth_values, num_depth, cost_regularization, prob_volume_init=None):
        ...
        ######################################################################################
        volume_count = torch.zeros(volume_sum.size())
        ######################################################################################
        ...
        for src_fea, src_proj in zip(src_features, src_projs):
            ...
            ref_proj_new[:, :3, :4] = torch.matmul(ref_proj[:, 1, :3, :3], ref_proj[:, 0, :3, :9])
            warped_volume = homo_warping(src_fea, src_proj_new, ref_proj_new, depth_values)

            ######################################################################################
            tmp_volume = torch.sum(abs(warped_volume), 1)
            tmp_volume = torch.eq(tmp_volume, 0)
            tmp_volume = (~tmp_volume).type('torch.FloatTensor')
            volume_count = volume_count + tmp_volume.repeat(1, volume_count.shape[1], 1, 1, 1)
            ######################################################################################
        ...
        #####################################setting3.1################################
        volume_count = volume_count.to('cuda')
        zero_index = torch.nonzero(volume_count == 0, as_tuple=True)
        #print("zero_index : ", len(zero_index[0]))
        volume_count[zero_index] = 25

        a = volume_sq_sum.div(volume_count)  #
        b = volume_sum.div(volume_count).pow_(2)
        volume_variance = a.sub(b)
        #####################################################################################
        ...
        #return {"depth": depth, "photometric_confidence": photometric_confidence,"zero_index": torch.tensor(len(zero_index[0])) }

class MosaicMVSNet(nn.Module):
    def __init__(self, refine=False, ndepths=[48, 32, 8], depth_interals_ratio=[4, 2, 1], share_cr=False,
        ...
        super(MosaicMVSNet, self).__init__()
        self.refine = refine
    ...
    def forward(self, imgs, proj_matrices, depth_values):
        ...
        for stage_idx in range(self.num_stage):
            ...
            #zero_index_dict["stage{}".format(stage_idx + 1)]=outputs_stage["zero_index"]
            outputs["stage{}".format(stage_idx + 1)] = outputs_stage["depth"], outputs_stage["photometric_confidence"]
            #outputs["stage{}".format(stage_idx + 1)] = outputs_stage["depth"], outputs_stage["photometric_confidence"],outputs_stage["zero_index"]
            outputs.update(outputs_stage)
        #print("zero_index_dict : ", zero_index_dict)
